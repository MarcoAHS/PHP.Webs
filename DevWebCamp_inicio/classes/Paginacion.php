...
class Paginacion{
    ...
    public function pagina_siguiente(){
        ...
        return ($siguente <= self::total_paginas()) ? $siguente : False;
    }
    ...
    public function numeros_paginas(){
        ...
        for($i = 1; $i <= $this->total_paginas(); $i++){
            if($this->pagina_actual === $i){$html .= "<span class=\"paginacion__enlace paginacion__enlace--actual\">${i}</span>";} 
            else {$html .= "<a class=\"paginacion__enlace\" href=\"?page=${i}\">${i}</a>";} 
        }
        return $html;
    }
}