...
<head>
    <meta charset="UTF-8">
    <title>Prueba Tecnica Usuarios</title>
    ...
    <!-- STYLES -->
            ...
            .contenedor-modal-new h2,
            .contenedor-modal-up h2{
                margin: 0 0 30px;
                padding: 0;
                color: #fff;
                text-align: center;
            }
            .contenedor-modal-new.mostrar,
            .contenedor-modal-up.mostrar{
                background-color: black;
                z-index: 1;
                opacity: 0.9;
            }
            .contenedor-modal-new .modal,
            .contenedor-modal-up .modal{
                width: 400px;
                padding: 40px;
            ...
            border: 3px solid rgba(221, 72, 20, .8);
            thead{
                border-bottom: 4px solid rgba(221, 72, 20, .8);
            }
            }
            tr:nth-child(even){
                background-color: #ccc;
...
<header>
    ...
    <div class="menu">
        <ul>
            ...
            <li class="logout"><a href="public/logout">Log Out</a></li>
        </ul>
    </div>
    ...
    <div class="heroe">
        ...
        <section class="main">
            <div class="filtros">
                <div class="limit">
                    <label for="limit">Limite de Datos por pagina:</label>
                ...
                <div class="search">
                    ...
                    <input type="text" id="search" name="search" placeholder="Que empleado buscas?">
                </div>
            </div>
            <table>
                <thead>
            ...
            <button id="lanzar-modal" type="submit">+ Nuevo Empleado</button>
            <div class="contenedor-modal-new">
                <form class="modal">
                    <img id="show" src="public/src/eye.svg" alt="Mostrar/Ocultar Password">
                    ...
                    <p>
                        <input name="submit" value="Agregar" type="submit" onclick="nuevoEmpleado();"/>
                        <button type="submit" id="ocultar-modal">Cancelar</button>
                    </p>
                </form>
            </div>
            <div class="contenedor-modal-up">
                <form class="modal">
                    <img id="show_u" src="public/src/eye.svg" alt="Mostrar/Ocultar Password">
                    <h2>Actualizar Info Empleado</h2>
                    <p>
                        <input hidden type="number" name="id" id="id_up">
                    </p>
                    <p class="campo">
                        <input required type="text" name="name" id="name_u">
                        <label for="name_u">Nombre:</label>
                    </p>
                    <p class="campo">
                        <input required type="text" name="phone" id="phone_u">
                        <label for="phone_u">Numero de Telefono:</label>
                    </p>
                    <p class="campo">
                        <input required type="email" name="email" id="email_u">
                        <label for="email_u">Correo Electronico:</label>
                    </p>
                    <p class="campo">
                        <input required type="password" name="password" id="password_u">
                        <label for="password_u">Contrasena:</label>
                    </p>
                    <p class="campo">
                        <input required type="date" name="date" id="date_u">
                        <label for="date_u">Fecha de Nacimiento:</label>
                    </p>
                    <p class="campo">
                        <input required type="text" name="rfc" id="rfc_u">
                        <label for="rfc_u">RFC:</label>
                    </p>
                    <p class="label">
                    ...
                    <p>
                        <input onclick="actualizarProducto();" name="submit" value="Actualizar" type="submit"/>
                        <button type="submit" id="ocultar-modal-up">Cancelar</button>
                    </p>
                </form>
            </div>
        </section>
...
<script {csp-script-nonce}>
    ...
    const btnOcultarModalUp = document.querySelector('#ocultar-modal-up');
    const btnShow = document.querySelector('#show');
    const btnShowU = document.querySelector('#show_u');
    const contModalNew = document.querySelector('.contenedor-modal-new');
    ...
    btnShowU.addEventListener('click', () => {
        password = document.querySelector('#password_u');
        if(document.querySelector('#password_u').type == 'text'){
            document.querySelector('#password_u').type = 'password'
        } else {
            document.querySelector('#password_u').type = 'text'
        }
    })
    btnLanzarModal.addEventListener('click', (e) => {
        e.preventDefault();
        contModalNew.classList.add('mostrar');
    })
    btnOcultarModal.addEventListener('click', (e) => {
        e.preventDefault();
        contModalNew.classList.remove('mostrar');
    })
    btnOcultarModalUp.addEventListener('click', (e) => {
        e.preventDefault();
        contModalUp.classList.remove('mostrar');
        document.getElementById('id_up').defaultValue = '';
    })
    ...
    function userInfo(id){
        $.get(`/public/user?id=${id}`,
            function(data) {
                ...
                document.querySelector('#name_u').value = datos.name
                document.querySelector('#phone_u').value = datos.phone
                document.querySelector('#email_u').value = datos.email
                document.querySelector('#password_u').value = datos.password
                document.querySelector('#date_u').value = datos.born_date
                document.querySelector('#rfc_u').value = datos.RFC
                document.querySelector('#type_u').value = datos.user_type
                contModalUp.classList.add('mostrar');
            }
        )
    }
