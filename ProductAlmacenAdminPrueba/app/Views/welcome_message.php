...
<head>
    <meta charset="UTF-8">
    <title>Prueba Tecnica CI4</title>
        ...
        <section class="main">
            <div class="filtros">
                ...
                <div class="search">
                    ...
                    <input type="text" id="search" name="search" placeholder="Que Producto estas buscando?">
                </div>
            </div>
            <table>
                <thead>
            ...
            <button id="lanzar-modal" type="submit">+ Nuevo Producto</button>
                ...
                <form class="modal">
                    ...
                    <p>
                        ...
                        <select name="id_almacen" id="id_almacen">
                            <option value="">--Selecciona el Almacen</option>
                    ...
                    <p>
                        <input name="submit" value="Agregar" type="submit" onclick="nuevoProducto();"/>
            ...
            <div class="contenedor-modal-up">
                <form class="modal">
                    ...
                    <p>
                        <label for="name">Nombre del Producto:</label>
                        <input type="text" name="name" id="name_u">
                    </p>
                    <p>
                        <label for="precio">Precio del Producto:</label>
                        <input type="number" name="precio" id="precio_u">
                    </p>
                    <p>
                        <label for="id_almacen">ID Del Almacen:</label>
                        <select name="id_almacen" id="id_almacen_u">
                            <option value="">--Selecciona el Almacen</option>
                            <?php foreach($almacenes as $almacen) { ?>
                                <option value="<?php echo $almacen->id; ?>"><?php echo $almacen->name; ?></option>
                            <?php } ?>
...
<script {csp-script-nonce}>
    ...
    function getData(page){
        ...
        $.get(`http://localhost/PruebaTecnicaCI4/project-root/public/consulta?limite=${limite}&page=${page}&search=${search}`,
            function(data) {
    ...
    function getPagination(page, limite){
        const search = $('#search').val() ?? '';
        $.get(`http://localhost/PruebaTecnicaCI4/project-root/public/pagination?page=${page}&limite=${limite}&search=${search}`,
            function(data) {
    ...
    function nuevoProducto(){
        ...
        $.ajax(
            {
                data: parametros,
                url: 'http://localhost/PruebaTecnicaCI4/project-root/public/nuevo',
                type: 'POST',
    ...
    function eliminarProducto(id){
        ...
        $.ajax(
            {
                data: parametros,
                url: 'http://localhost/PruebaTecnicaCI4/project-root/public/eliminar',
                type: 'POST',
                error: function(){alert('Error');},
                success: function(value) {
                    getData(page);
    ...
    function leerProducto(id){
        $.get(`http://localhost/PruebaTecnicaCI4/project-root/public/producto?id=${id}`,
            function(data) {
                datos = $.parseJSON(data);
                document.querySelector('#id_up').value = datos.id
                document.querySelector('#name_u').value = datos.name
                document.querySelector('#precio_u').value = datos.precio
                document.querySelector('#id_almacen_u').value = datos.id_almacen
    ...
    function actualizarProducto(){
        var parametros = {
            ...
            "name" : $('#name_u').val(),
            "precio" : $('#precio_u').val(),
            "id_almacen" : $('#id_almacen_u').val()
        }
        $.ajax(
            {
                data: parametros,
                url: 'http://localhost/PruebaTecnicaCI4/project-root/public/actualizar',
                type: 'POST',
                error: function(){alert('Error');},
                success: function(value) {
                    getData(page);
